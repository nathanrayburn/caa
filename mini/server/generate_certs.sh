...
KEY_STORE_DIR="certs/keystore"

# Create the directories if they don't exist
mkdir -p "$PUBLIC_DIR" "$PRIVATE_DIR" "$KEY_STORE_DIR"
...
PRIVATE_KEY_PATH="$PRIVATE_DIR/key.pem"
CERT_PATH="$PUBLIC_DIR/cert.pem"

openssl pkcs12 -export -out "$KEY_STORE_DIR/keyStore.p12" -inkey "$PRIVATE_KEY_PATH" -in "$CERT_PATH"
